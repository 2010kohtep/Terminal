...
const
  ...
  FCMD_CLIENTDLL = 1 shl 0;
  FCMD_SERVER = 1 shl 1;
  FCMD_WRAPPER = 1 shl 2;

  ACTIVITY_NOT_AVAILABLE = -1;

type
 Activity = (ACT_RESET = 0, ACT_IDLE = 1, ACT_GUARD, ACT_WALK, ACT_RUN,
 ...
 ALERT_TYPE = (at_notice, at_console, at_aiconsole, at_warning, at_error, at_logged);
 PRINT_TYPE = (print_console, print_center, print_chat);
 FORCE_TYPE = (force_exactfile, force_model_samebounds, force_model_specifybounds,
               force_model_specifybounds_if_avail);

 FIELDTYPE = (FIELD_FLOAT = 0, FIELD_STRING, FIELD_ENTITY, FIELD_CLASSPTR, FIELD_EHANDLE,
 ...
 ptype_t = (pt_static, pt_grav, pt_slowgrav, pt_fire, pt_explode, pt_explode2, pt_blob, pt_blob2,
            pt_vox_slowgrav, pt_vox_grav, pt_clientcustom);

 VoiceTweakControl = (MicrophoneVolume = 0, OtherSpeakerScale, MicBoost);

 TRICULLSTYLE = (TRI_FRONT = 0, TRI_NONE = 1);

 cactive_s = ^cactive_t;
 cactive_t = (ca_dedicated = 0, ca_disconnected, ca_connecting, ca_connected, ca_uninitialized, ca_active);

 ECAM_Command = (CAM_COMMAND_NONE = 0, CAM_COMMAND_TOTHIRDPERSON = 1, CAM_COMMAND_TOFIRSTPERSON = 2);
 USE_TYPE = (USE_OFF = 0, USE_ON = 1, USE_SET = 2, USE_TOGGLE = 3);

 decal_e = (DECAL_GUNSHOT1 = 0, DECAL_GUNSHOT2, DECAL_GUNSHOT3, DECAL_GUNSHOT4,
 ...
 TANKBULLET = (TANK_BULLET_NONE = 0, TANK_BULLET_9MM = 1, TANK_BULLET_MP5 = 2, TANK_BULLET_12MM = 3);
 GLOBALENTSTATE = (GLOBAL_OFF = 0, GLOBAL_ON = 1, GLOBAL_DEAD = 2);
 MONSTERSTATE = (MONSTERSTATE_NONE = 0, MONSTERSTATE_IDLE, MONSTERSTATE_COMBAT,
                 MONSTERSTATE_ALERT, MONSTERSTATE_HUNT, MONSTERSTATE_PRONE,
                 MONSTERSTATE_SCRIPT, MONSTERSTATE_PLAYDEAD, MONSTERSTATE_DEAD);
 TOGGLE_STATE = (TS_AT_TOP, TS_AT_BOTTOM, TS_GOING_UP, TS_GOING_DOWN);

 cmd_source_t = (src_client, src_command);

 modtype_t = (mod_brush = 0, mod_sprite, mod_alias, mod_studio);
 synctype_t = (ST_SYNC = 0, ST_RAND);

 netsrc_t = (NS_CLIENT = 0, NS_SERVER, NS_LISTEN);
 ...
 sequenceCommandType_e = (SEQUENCE_TYPE_COMMAND = 0, SEQUENCE_TYPE_MODIFIER);
...
type
 ...
 TDeltaField = record
  FieldType: LongWord;          // 0. Confirmed.
  Name: array[1..32] of Char; // 4. Confirmed. A field name.
  Offset: LongWord;             // 36. Confirmed. Offset (unsigned).
  Parsed: Word;                 // 40. Sets to "1" when parsing.
  Bits: LongWord;               // 44. Confirmed. How many bits are in offset value.
  ...
  Flags: set of (ffReady, __ffPadding = 15);    // 56. Unsure about this.
                                                //     Is 16-bit, actually.
  SendCount: LongWord;          // 60. How many times we should "send" this field.
  RecvCount: LongWord;         // 64. Delta_Parse increments it.
 end;
 ...
 TDelta = record
  ...
  NumFields: LongInt; // 4: Number of delta fields. Signed.
  Name: array[1..32] of Char; // 8. Confirmed.
  ConditionalEncoder: TDeltaEncoder; // 40. Confirmed.
  Fields: ^TDeltaFieldArray; // 44: Pointer to a field array.
 end;

 //delta_s = PDelta;
 PRejectArr = ^TRejectArr;
...
type
 ...
 player_info_t = record
  UserID: Longint;
  UserInfo: array[1..256] of Char;
  Name: array[1..32] of Char;
  Spectator: Longint;
  Ping: Longint;
  PacketLoss: Longint;
  Model: array[1..64] of Char;
  TopColor: Longint;
  BottomColor: Longint;
  RenderFrame: Longint;
  GaitSequence: Longint;
  GaitFrame: Single;
  GaitYaw: Single;
  PrevGaitOrigin: array[1..3] of Single;
  CustomData: customization_t;
  UniqueID: TUPlayerID;
  SteamID: array[1..8] of Byte;
end;
...
type ConditionalEncode = procedure(const Fields: delta_s; const From, Destination: PChar); cdecl;
...
type
 ...
 enginefuncs_t = record
  ...
  SetModel: procedure(Entity: edict_s; ModelName: PChar); cdecl;
  ModelIndex: function(ModelName: PChar): Longint; cdecl;
  ModelFrames: function(ModelIndex: Longint): Longint; cdecl;
  SetSize: procedure(Entity: edict_s; Min, Max: vec3_s); cdecl;
  ChangeLevel: procedure(S1, S2: PChar); cdecl;
  GetSpawnParms: procedure(Entity: edict_s); cdecl;
  SaveSpawnParms: procedure(Entity: edict_s); cdecl;
  VecToYaw: function(Vector: vec3_s): Single; cdecl;
  VecToAngles: procedure(VectorIn, VectorOut: vec3_s); cdecl;
  MoveToOrigin: procedure(Entity: edict_s; Target: vec3_s; Distance: Single; MoveType: Longint); cdecl;
  ChangeYaw: procedure(Entity: edict_s); cdecl;
  ChangePitch: procedure(Entity: edict_s); cdecl;
  FindEntityByString: function(StartFrom: edict_s; Key, Value: PChar): edict_s; cdecl;
  GetEntityIllum: function(Entity: edict_s): Longint; cdecl;
  FindEntityInSphere: function(StartFrom: edict_s; Origin: vec3_s; Distance: Single): edict_s; cdecl;
  FindClientInPVS: function(Entity: edict_s): edict_s; cdecl;
  EntitiesInPVS: function(Entity: edict_s): edict_s; cdecl;
  MakeVectors: procedure(Vector: vec3_s); cdecl;
  AngleVectors: procedure(Vector, VectorForward, VectorRight, VectorUp: vec3_s); cdecl;
  CreateEntity: function: edict_s; cdecl;
  RemoveEntity: procedure(Entity: edict_s); cdecl;
  CreateNamedEntity: function(ClassName: Longint): edict_s; cdecl;
  MakeStatic: procedure(Entity: edict_s); cdecl;
  EntIsOnFloor: function(Entity: edict_s): Longint; cdecl;
  DropToFloor: function(Entity: edict_s): Longint; cdecl;
  WalkMove: function(Entity: edict_s; Yaw, Distance: Single; Mode: Longint): Longint; cdecl;
  SetOrigin: procedure(Entity: edict_s; Origin: vec3_s); cdecl;
  EmitSound: procedure(Entity: edict_s; Channel: Longint; Sample: PChar; Volume, Attenuation: Single; Flags, Pitch: Longint); cdecl;
  EmitAmbientSound: procedure(Entity: edict_s; Origin: vec3_s; Sample: PChar; Volume, Attenuation: Single; Flags, Pitch: Longint); cdecl;
  TraceLine: procedure(V1, V2: vec3_s; NoMonsters: Longint; EntityToSkip: edict_s; TraceResult: PTraceResult); cdecl;
  TraceToss: procedure(Entity: edict_s; EntityToSkip: edict_s; TraceResult: PTraceResult); cdecl;
  TraceMonsterHull: function(Entity: edict_s; V1, V2: vec3_s; NoMonsters: Longint; EntityToSkip: edict_s; TraceResult: PTraceResult): Longint; cdecl;
  TraceHull: procedure(V1, V2: vec3_s; NoMonsters, HullNumber: Longint; EntityToSkip: edict_s; TraceResult: PTraceResult); cdecl;
  TraceModel: procedure(V1, V2: vec3_s; HullNumber: Longint; Entity: edict_s; TraceResult: PTraceResult); cdecl;
  TraceTexture: function(Entity: edict_s; V1, V2: vec3_s): PChar; cdecl;
  TraceSphere: procedure(V1, V2: vec3_s; NoMonsters: Longint; Radius: Single; EntityToSkip: edict_s; TraceResult: PTraceResult); cdecl;
  GetAimVector: procedure(Entity: edict_s; Speed: Single; VectorReturn: vec3_s); cdecl;
  ServerCommand: procedure(S: PChar); cdecl;
  ServerExecute: procedure; cdecl;
  ClientCommand: procedure(Entity: edict_s; S: PChar); cdecl varargs;
  ...
  PointContents: function(Vector: vec3_s): Longint; cdecl;
  MessageBegin: procedure(Destination, MessageType: Longint; Origin: vec3_s; Entity: edict_s); cdecl;
  ...
  CVarRegister: procedure(CVar: cvar_s); cdecl;
  CVarGetFloat: function(Name: PChar): Single; cdecl;
  CVarGetString: function(Name: PChar): PChar; cdecl;
  CVarSetFloat: procedure(Name: PChar; Value: Single); cdecl;
  CVarSetString: procedure(Name, Value: PChar); cdecl;
  AlertMessage: procedure(AlertType: ALERT_TYPE; Msg: PChar); cdecl varargs;
  EngineFPrintF: procedure(F: Pointer; S: PChar); cdecl varargs;
  PVAllocEntPrivateData: function(Entity: edict_s; CB: LongWord): Pointer; cdecl;
  PVEntPrivateData: function(Entity: edict_s): Pointer; cdecl;
  FreeEntPrivateData: procedure(Entity: edict_s); cdecl;
  SZFromIndex: function(Index: Longint): PChar; cdecl;
  AllocString: function(S: PChar): Longint; cdecl;
  GetVarsOfEnt: function(Entity: edict_s): entvars_s; cdecl;
  PEntityOfEntOffset: function(Offset: Longint): edict_s; cdecl;
  EntOffsetOfPEntity: function(Entity: edict_s): Longint; cdecl;
  IndexOfEdict: function(Entity: edict_s): Longint; cdecl;
  PEntityOfEntIndex: function(Index: Longint): edict_s; cdecl;
  FindEntityByVars: function(Vars: entvars_s): edict_s; cdecl;
  GetModelPtr: function(Entity: edict_s): Pointer; cdecl;
  RegUserMsg: function(Name: PChar; Size: Longint): Longint; cdecl;
  AnimationAutomove: procedure(Entity: edict_s; Time: Single); cdecl;
  GetBonePosition: procedure(Entity: edict_s; Bone: Longint; Origin, Angles: vec3_s); cdecl;
  FunctionFromName: function(Name: PChar): LongWord; cdecl;
  NameForFunction: function(Func: LongWord): PChar; cdecl;
  ClientPrintF: procedure(Entity: edict_s; PrintType: PRINT_TYPE; Msg: PChar); cdecl;
  ...
  GetAttachment: procedure(Entity: edict_s; Attachment: Longint; Origin, Angles: vec3_s); cdecl;
  CRC32_Init: procedure(CRC: CRC32_s); cdecl;
  CRC32_ProcessBuffer: procedure(CRC: CRC32_s; Ptr: Pointer; Length: Longint); cdecl;
  CRC32_ProcessByte: procedure(CRC: CRC32_s; C: Char); cdecl;
  CRC32_Final: function(CRC: CRC32_t): CRC32_t; cdecl;
  RandomLong: function(Low, High: LongWord): LongWord; cdecl;
  RandomFloat: function(Low, High: Single): Single; cdecl;
  SetView: procedure(Entity, Target: edict_s); cdecl;
  Time: function: Double; cdecl;
  CrosshairAngle: procedure(Entity: edict_s; Pitch, Yaw: Single); cdecl;
  ...
  FadeClientVolume: procedure(Entity: edict_s; FadePercent, FadeoutSeconds, HoldTime, FadeinSeconds: Longint); cdecl;
  SetClientMaxSpeed: procedure(Entity: edict_s; Speed: Single); cdecl;
  CreateFakeClient: function(Name: PChar): edict_s; cdecl;
  RunPlayerMove: procedure(Entity: edict_s; Angles: vec3_s; ForwardMove, SideMove, UpMove: Single; Buttons: Smallint; Impulse, MSec: Byte); cdecl;
  NumberOfEntities: function: Longint; cdecl;
  GetInfoKeyBuffer: function(Entity: edict_s): PChar; cdecl;
  ...
  PrecacheGeneric: function(S: PChar): Longint; cdecl;
  GetPlayerUserID: function(Entity: edict_s): Longint; cdecl;
  BuildSoundMsg: procedure(Entity: edict_s; Channel: Longint; Sample: PChar; Volume, Attenuation: Single; Flags, Pitch, Msg_Dest, Msg_Type: Longint; Origin: vec3_s; Entity2: edict_s); cdecl;
  IsDedicatedServer: function: Longint; cdecl;
  CVarGetPointer: function(Name: PChar): cvar_s; cdecl;
  GetPlayerWONID: function(Entity: edict_s): Longint; cdecl;
  Info_RemoveKey: procedure(Data: PChar; Key: PChar); cdecl;
  GetPhysicsKeyValue: function(Entity: edict_s; Key: PChar): PChar; cdecl;
  SetPhysicsKeyValue: procedure(Entity: edict_s; Key: PChar; Value: PChar); cdecl;
  GetPhysicsInfoString: function(Entity: edict_s): PChar; cdecl;
  PrecacheEvent: function(EventType: Longint; Name: PChar): Smallint; cdecl;
  PlaybackEvent: procedure(Flags: Longint; Entity: edict_s; EventIndex: Smallint; Delay: Single; Origin, Angles: vec3_s; F1, F2: Single; I1, I2, B1, B2: Longint); cdecl;
  SetFatPVS: function(Origin: vec3_s): PChar; cdecl;
  SetFatPAS: function(Origin: vec3_s): PChar; cdecl;
  CheckVisibility: function(Entity: edict_s; VisibilitySet: PChar): Longint; cdecl;
  ...
  DeltaAddEncoder: procedure(Name: PChar; ConditionalEncode: ConditionalEncode); cdecl;
  GetCurrentPlayer: function: Longint; cdecl;
  CanSkipPlayer: function(Entity: edict_s): Longint; cdecl;
  DeltaFindField: function(Fields: delta_s; Name: PChar): Longint; cdecl;
  ...
  ForceUnmodified: procedure(ForceType: FORCE_TYPE; MinS, MaxS: PSingle; FileName: PChar); cdecl;
  GetPlayerStats: procedure(Entity: edict_s; Ping: PLongint; PacketLoss: PLongint); cdecl;
  AddServerCommand: procedure(Name: PChar; Callback: TCallback); cdecl;
  ...
  GetPlayerAuthID: function(Entity: edict_s): PChar; cdecl;
  ...
  GetLocalizedStringLength: function(S: PChar): Longint; cdecl;
  ...
  QueryClientCvarValue: procedure(Entity: edict_s; Name: PChar); cdecl;
  QueryClientCvarValue2: procedure(Entity: edict_s; Name: PChar; RequestID: Longint); cdecl;
...
type
 ...
 playermove_t = record
  ...
  GetMemory: function(pMemFile: PByte; FileSize: Longint; pFilePos: PLongint; pBuffer: PChar; BufferSize: Longint): PChar; cdecl;
  RunFuncs: Longint;
...
type
 ...
 DLL_FUNCTIONS = record
  ...
  Spawn: function(const Entity: edict_s): Longint; cdecl;
  Think: procedure(const Entity: edict_s); cdecl;
  Use: procedure(const EntityUsed, EntityOther: edict_s); cdecl;
  Touch: procedure(const EntityTouched, EntityOther: edict_s); cdecl;
  Blocked: procedure(const EntityBlocked, EntityOther: edict_s); cdecl;
  KeyValue: procedure(const Entity: edict_s; const Data: PKeyValueData); cdecl;
  Save: procedure(const Entity: edict_s; const SaveData: PSAVERESTOREDATA); cdecl;
  Restore: function(const Entity: edict_s; const SaveData: PSAVERESTOREDATA; GlobalEntity: Longint): Longint; cdecl;
  SetAbsBox: procedure(const Entity: edict_s); cdecl;
  SaveWriteFields: procedure(const SaveData: PSAVERESTOREDATA; const Name: PChar; const BaseData: Pointer; const Fields: PTYPEDESCRIPTION; FieldCount: Longint); cdecl;
  SaveReadFields: procedure(const SaveData: PSAVERESTOREDATA; const Name: PChar; const BaseData: Pointer; const Fields: PTYPEDESCRIPTION; FieldCount: Longint); cdecl;
  SaveGlobalState: procedure(const SaveData: PSAVERESTOREDATA); cdecl;
  RestoreGlobalState: procedure(const SaveData: PSAVERESTOREDATA); cdecl;
  ResetGlobalState: procedure; cdecl;
  ClientConnect: function(const Entity: edict_s; const Name, Address: PChar; const RejectReason: PRejectArr): Longint; cdecl;
  ClientDisconnect: procedure(const Entity: edict_s); cdecl;
  ClientKill: procedure(const Entity: edict_s); cdecl;
  ClientPutInServer: procedure(const Entity: edict_s); cdecl;
  ClientCommand: procedure(const Entity: edict_s); cdecl;
  ClientUserInfoChanged: procedure(const Entity: edict_s; const Buffer: PChar); cdecl;
  ServerActivate: procedure(const EntityList: edict_s; EntityCount, MaxClients: Longint); cdecl;
  ServerDeactivate: procedure; cdecl;
  PlayerPreThink: procedure(const Entity: edict_s); cdecl;
  PlayerPostThink: procedure(const Entity: edict_s); cdecl;
  ...
  PlayerCustomization: procedure(const Entity: edict_s; const Custom: customization_s); cdecl;
  SpectatorConnect: procedure(const Entity: edict_s); cdecl;
  SpectatorDisconnect: procedure(const Entity: edict_s); cdecl;
  SpectatorThink: procedure(const Entity: edict_s); cdecl;
  Sys_Error: procedure(const ErrorString: PChar); cdecl;
  PM_Move: procedure(const PlayerMove: playermove_s; Server: Longint); cdecl;
  PM_Init: procedure(const PlayerMove: playermove_s); cdecl;
  PM_FindTextureType: function(const Name: PChar): Char; cdecl;
  SetupVisibility: procedure(const Target, Entity: edict_s; const PVS, PAS: PPByte); cdecl;
  UpdateClientData: procedure(const Entity: edict_s; SendWeapons: Longint; const ClientData: clientdata_s); cdecl;
  AddToFullPack: function(const State: entity_state_s; E: Longint; const Entity, Host: edict_s; HostFlags, Player: Longint; const PackSet: PByte): Longint; cdecl;
  CreateBaseline: procedure(Player, EntityIndex: Longint; const BaseLine: entity_state_s; const Entity: edict_s; PlayerModelIndex: Longint; const Player_MinS, Player_MaxS: vec3_s); cdecl;
  RegisterEncoders: procedure; cdecl;
  GetWeaponData: function(const Entity: edict_s; const Info: weapon_data_s): Longint; cdecl;
  CmdStart: procedure(const Entity: edict_s; const Command: usercmd_s; RandomSeed: LongWord); cdecl;
  CmdEnd: procedure(const Entity: edict_s); cdecl;
  ConnectionlessPacket: function(const Address: netadr_s; const Args: PChar; const ResponseBuffer: PChar; const ResponseBufferSize: PLongint): Longint; cdecl;
  GetHullBounds: function(HullNumber: Longint; const MinS, MaxS: vec3_s): Longint; cdecl;
  CreateInstancedBaselines: procedure; cdecl;
  InconsistentFile: function(const Entity: edict_s; const FileName, DisconnectMessage: PChar): Longint; cdecl;
...
type
 ...
 NEW_DLL_FUNCTIONS = record
  OnFreeEntPrivateData: procedure(const Entity: edict_s); cdecl;
  GameShutdown: procedure; cdecl;
  ShouldCollide: function(const EntityTouched, EntityOther: edict_s): LongWord; cdecl;
  CVarValue: procedure(const Entity: edict_s; const Value: PChar); cdecl;
  CVarValue2: procedure(const Entity: edict_s; RequestID: Longint; const CvarName, Value: PChar); cdecl;
end;

type NEW_DLL_FUNCTIONS_FN = function(const FunctionTable: PNEW_DLL_FUNCTIONS; const InterfaceVersion: PLongint): Longint; cdecl;

type APIFUNCTION = function(const FunctionTable: PDLL_FUNCTIONS; const InterfaceVersion: Longint): Longint; cdecl;
type APIFUNCTION2 = function(const FunctionTable: PDLL_FUNCTIONS; const InterfaceVersion: PLongint): Longint; cdecl;
...
type
 ...
 fragbuf_t = record
  Next: fragbuf_s;
  BufferID: Longint;
  FragMessage: sizebuf_t;
  FragMessageBuffer: array[1..FRAGMENT_SIZE] of Byte;
  IsFile, IsBuffer, IsCompressed: Longint;
  FileName: array[1..260] of Char;
  FileOffset: Longint;
  Size: Longint;
 end;

 fragbufwaiting_s = ^fragbufwaiting_t;
 fragbufwaiting_t = record
  Next: fragbufwaiting_s;
  Count: Longint;
  Buffer: fragbuf_s;
 end;

 flowstats_s = ^flowstats_t;
 flowstats_t = record
  Size: Longint;
  Time: Single;
 end;
 ...
 flow_s = ^flow_t;
 flow_t = record
  Stats: array[1..MAX_LATENT] of flowstats_t;
  Current: Longint;
  NextCompute: Double;
  KBPerSecond: Single;
  AvgKBPerSecond: Single;
  TotalBytes: Longint;
 end;
 ...
 netchan_t = record
  ...
  RemoteAddress: netadr_t;
  Port: Longint;
  LastReceived, ConnectTime: Single;
  Rate, ClearTime: Double;
  IncomingSeq, IncomingAck, IncomingReliableSeq, IncomingReliableAck: Longint;
  OutgoingSeq, ReliableSeq, LastReliableSeq: Longint;
  ConnectionState: cactive_s;
  FragCallback: function(const State: cactive_s): Longint; cdecl;
  MessageInfo: sizebuf_t;
  MessageBuffer: array[1..NET_MAX_PAYLOAD] of Byte;
  ReliableLength: Longint;
  ReliableBuffer: array[1..NET_MAX_PAYLOAD] of Byte;
  WaitList: array[1..MAX_STREAMS] of fragbufwaiting_s;
  ReliableFragment: array[1..MAX_STREAMS] of Longint;
  ReliableFragID: array[1..MAX_STREAMS] of LongWord;
  FragBuf: array[1..MAX_STREAMS] of fragbuf_s;
  FragBufCount: array[1..MAX_STREAMS] of Longint;
  FragStartPos: array[1..MAX_STREAMS] of Smallint;
  FragLength: array[1..MAX_STREAMS] of Smallint;  
  IncomingBuffer: array[1..MAX_STREAMS] of fragbuf_s;
  IncomingReady: array[1..MAX_STREAMS] of Longint;
  IncomingFileName: array[1..260] of Char;
  Flow: array[1..MAX_FLOWS] of flow_t;
end;
...
type
 ...
 cl_enginefuncs_t = record
  ...
  GetPlayerUniqueID: function(Index: Longint; ID: PUPlayerID): Longint; cdecl;
  ...
  GetPrevFrameTime: function: Double; cdecl;
  GetGravity: function: Single; cdecl;
  GetModelByIndex: function(Index: Longint): model_s; cdecl;
  SetLightMapState: procedure(Disable: Longint); cdecl;
  SetLightMapColor: procedure(R, G, B: Single); cdecl;
  SetLightMapDarkness: procedure(Darkness: Single); cdecl;
  SequenceGet: function(const FileName, EntryName: PChar): sequenceEntry_s; cdecl;
  SPR_DrawGeneric: procedure(Frame, X, Y: Longint; const Rect: wrect_s; BlendSrc, BlendDst, Width, Height: Longint); cdecl;
  SequencePickSentence: function(const GroupName: PChar; PickMethod: Longint; const Picked: PLongint): sentenceEntry_s; cdecl;
  ...
  LocalInfo_ValueForKey: function(const Key: PChar): PChar; cdecl;
  DrawVGUICharacter: function(X, Y: Longint; C: Char; Font: LongWord): Longint; cdecl;
  DrawVGUICharacter2: function(X, Y: Longint; C: Char; R, G, B: Longint; Font: LongWord): Longint; cdecl;
  ...
  Sys_FloatTime: function: Double; cdecl;
  ProcessTutorMessageDecayBuffer: procedure(const Buffer: PLongint; Length: Longint); cdecl;
  ConstructTutorMessageDecayBuffer: procedure(const Buffer: PLongint; Length: Longint); cdecl;
  ResetTutorMessageDecayData: function: Longint; cdecl;
  PlayItemSound: procedure(const Name: PChar; Volume: Single; Pitch: Longint); cdecl;
...
type
 ...
 exporttable_t = record
  Initialize: function(Engine: cl_enginefuncs_s; Version: Longint): Longint; cdecl;
  HUD_Init: function: Longint; cdecl;
  HUD_VidInit: function: Longint; cdecl;
  HUD_Redraw: function(Time: Single; Intermission: Longint): Longint; cdecl;
  HUD_UpdateClientData: function(Data: client_data_s; Time: Single): Longint; cdecl;
  HUD_Reset: function: Longint; cdecl;
  HUD_PlayerMove: procedure(PlayerMove: playermove_s; Server: Longint); cdecl;
  ...
  CL_CreateMove: procedure(FrameTime: Single; Command: usercmd_s; Active: Longint); cdecl;
  ...
  HUD_AddEntity: function(EntityType: Longint; pEnt: cl_entity_s; ModelName: PChar): Longint; cdecl;
  ...
  HUD_Key_Event: function(Down, KeyNum: Longint; const Binding: PChar): Longint; cdecl;
...
type
 ...
 NET_SendPacket = procedure(HostType: netsrc_t; Length: LongWord; const Buffer: Pointer; Addr: netadr_t); cdecl;
 ...
 CL_CheckFile = function(const Buffer: sizebuf_s; const ResourceName: PChar): Longint; cdecl;
 ...
 Key_Message = procedure(Key: Longint); cdecl;
 Voice_Init = function(const CodecName: PChar; Quality: Longint): Longint; cdecl;
 COM_ParseLine = function(const Data: PChar): Pointer; cdecl;
 SV_SendVoiceInit = procedure(const Buffer: sizebuf_s); cdecl;
 Netchan_CreateFileFragments = function(Server: Longint; const Channel: netchan_s; const FileName: PChar): Longint; cdecl;
 Netchan_SendFragments = procedure(const Channel: netchan_s); cdecl;

type
 COM_Token = array[1..1024] of Char;
 cmd_arg_array_s = ^cmd_arg_array_t;
 cmd_arg_array_t = array[1..MAX_ARGS] of PChar; 
...
type
 ...
 MSG_ReadBuffer = function(Size: LongWord; const Buffer: Pointer): Longint; cdecl;
 ...
 MSG_WriteLong = procedure(const Buffer: sizebuf_s; Value: LongWord); cdecl;
...
type
 particleman_api_s = ^particleman_api_t;
 particleman_api_t = record
  ClassHandler: function(Dispose: Boolean): Pointer; stdcall; // 0
  Initialize: procedure(const Engine: cl_enginefuncs_s); stdcall; // 4
  Frame: procedure; stdcall; // 8
  SetParameters: procedure(Gravity: Single; ViewX, ViewY, ViewZ: Longint); stdcall; // 12
  Reset: procedure; stdcall; // 16
  FillPMStats: function(P1, P2, P3, P4, P5, P6, P7, P8: LongWord; ClientTime: Single): Pointer; stdcall; // 20; unused
  SetPMMode: procedure(Mode: Longint); stdcall; // 24
 end;
...
type
 ...
 VHUDAmmo = record
  ...
  VidInit: function: Longint; stdcall;
  Draw: function(Time: Single): Longint; stdcall;
  Think: procedure; stdcall;
  Reset: procedure; stdcall;
  InitHUDData: procedure; stdcall;
 end;

 DHUDAmmo = record
  X, Y, HUDType, Flags: Longint;
  Fade: Single;
  Color: packed array[1..4] of Byte;
  Weapon: PWEAPON;
  HUDBucket0: Longint;
  HUDSelection: Longint;
 end;
...
type
 ...
 TUserCmd = packed record
  ...
  ImpactIndex: LongInt; // 36
  ImpactPosition: vec3_t; // 40
 end;
